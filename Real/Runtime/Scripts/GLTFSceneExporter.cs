...
namespace UnityGLTF
{
	...
	public class GLTFSceneExporter
	{
		...
		private byte[] GetStreamBin(Stream stream)
		{
			stream.Position = 0;
			var bytes = SerializeToBytes(stream);
			// glbData = Encoding.Default.GetString(bytes);
		...
		public void SaveGLBToStream(Stream stream, string sceneName, bool bin)
		{
			...
			var glbLength = (int)(GLTFHeaderSize + SectionHeaderSize +
